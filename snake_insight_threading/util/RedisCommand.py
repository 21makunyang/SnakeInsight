...
class RedisCommand(object):
    ...
    def hset(self, info: HouseInfo):
        self.redis.hset(name=f'{info.city}:{info.region}:{info.area}:{info.id}', mapping=info.__dict__)

    def hgetall(self, id_):
    ...
    def scan(self,
        ...
        count - 指定查询元素数量，默认值为 10。（count 不是限定返回结果的数量，而是限定服务器单次遍历的字典槽位数量，所以返回数量可能因为 match 过滤而返回少于 count）
